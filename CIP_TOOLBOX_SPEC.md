...
**功能**：用條碼機逐張掃描包裝好的物件貼紙，取代人工對清單清點；掃滿一組才可封箱。

**輸入**：加工過的統單貼紙 xlsx，必要欄位 `條碼內容`、`分箱組`，另用 `流水號`、`櫃點`、`櫃型`、`米數`、`訂單`、`寄送`。自動掃描所有分頁，取第一個同時具備「條碼內容」與「分箱組」的分頁。

**分箱規則**（產檔階段決定，本工具只讀 `分箱組` 欄）：
有業務名 → 該業務一箱；業務欄為空／`0`／`#N/A` → 依 `寄送` 分區成箱（組名 `區-XX`）。寄送值需 TRIM，否則 ` C` 會自成一組。
